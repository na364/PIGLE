...
% unitcell - a struct with the following fields:
%   numOfPrmtvCells - number of primitive cells in the unitcell (along the
%   lateral dimensions) - a vector [n_x n_y]
%   x,y,z,theta,tilt - vectors containing the grid
...
if ~params.theta_enabled && isfield('theta',params.unitcell)
    ...
    params.unitcell.celldim(4)=[];
end
if ~params.thetatilt_enabled && isfield('tilt',params.unitcell)
    params.unitcell=rmfield(params.unitcell,'tilt');
    params.unitcell.celldim(5)=[];
end
...
numangmass=size(params.angular_mass_list);
if length(params.number_density) ~= length(params.mass_list) ...
        || numangmass(1) ~= length(params.mass_list)
    error('number_density, mass_list, and angular_mass_list - must have the same length lengthes')
end
...
[r_conf(1:Nmass).diatomic_mass]                   = deal(r_conf_diatomic_mass{:});
[r_conf(1:Nmass).is_vertical]                     = deal(r_conf_vertical{:});
...
% calculate_sim_params, also amend surface_params.m
...
% then the 'z' dimension is assumed to be 3rd. After this, the following dimensions
% are angular if enabled (theta followed by tilt).
...
for i=1:length(params.mass_list)
    if params.prtcl(i).A_spatial_depended_friction == 1
        ...
        PESlike = prepFuncs.adjust_PES(params,PESlike); % reduce PotMatrix to match the enabled dimensions
...
% Depending on the case (see calc_A function in calculate_sim_params.m),
% different fields of the structure A_struct are expected.
[A_tilt_strct(1:Nmass).A_case] = deal(A_tilt_case{:});
[A_tilt_strct(1:Nmass).w0]     = deal(A_tilt_w0{:});
[A_tilt_strct(1:Nmass).dw]     = deal(A_tilt_dw{:});
[A_tilt_strct(1:Nmass).eta]    = deal(A_tilt_eta{:});
[A_tilt_strct(1:Nmass).tau]    = deal(A_tilt_tau{:});

[params.prtcl(1:Nmass).A_spatial_depended_tilt_friction]    = deal(A_spatial_depended_tilt_friction{:});
...
for i=1:length(params.mass_list)
    if params.prtcl(i).A_spatial_depended_tilt_friction == 1
        [PESlike] = tilt_friction_func_list{i}(tilt_friction_arg_list{i,:});
        PESlike = prepFuncs.adjust_PES(params,PESlike); % resuce PotMatrix to match the enabled dimensions
        params.prtcl(i).friction.tilt_scaleMat = PESlike;
    else
        params.prtcl(i).friction.tilt_scaleMat = 1;
    end
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% Order of dimensions: 1st - 'y', 2nd - 'x'. If 'z' dimension is enabled,
% then the 'z' dimension is  3rd. After this, the following dimensions are
% assumed to be angular in the order theta, tilt.
% So for example, PotMatrix(i,j) is the surface coordinate (j,i).
for i=1:length(params.mass_list)
    [PotMatrix] = PES_func_list{i}(PES_arg_list{i,:});
    PotMatrix = prepFuncs.adjust_PES(params,PotMatrix); % reduce PotMatrix to match the enabled dimensions
    ...
    elseif params.model_dim == 4 && params.z_enabled
        dz = diff(params.unitcell.z(1:2)); dtheta = diff(params.unitcell.theta(1:2));
        [fx,fy,fz,ftheta] = gradient(PotMatrix,dy,dx,dz,dtheta);
    elseif params.model_dim == 4 && params.thetatilt_enabled
        dtheta=diff(params.unitcell.theta(1:2)); dtilt=diff(params.unitcell.tilt(1:2));
        [fx,fy,ftheta,ftilt]=gradient(PotMatrix,dy,dx,dtheta,dtilt);
    else
        dz=diff(params.unitcell.z(1:2)); dtheta=diff(params.unitcell.theta(1:2));
        dtilt=diff(params.unitcell.tilt(1:2));
        [fx,fy,fz,ftheta,ftilt]=gradient(PotMatrix,dy,dx,dz,dtheta,dtilt);
    end
    ...
    if exist('ftilt','var'), params.prtcl(i).pes.ftilt=-ftilt*k1; end
end
